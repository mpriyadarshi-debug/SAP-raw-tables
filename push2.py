...
for table in TABLES:
    try:
        ...
        if job.status_code not in (200,201):
            print("  X " + table + " job: " + str(job.status_code) + " " + job.text[:120]); fail+=1; continue
        ...
        if chunk.status_code not in (200,201,204):
            print("  X " + table + " chunk: " + str(chunk.status_code) + " " + chunk.text[:120]); fail+=1; continue
        ...
        if exe.status_code in (200,201,204):
            ...
            print("  X " + table + " execute: " + str(exe.status_code) + " " + exe.text[:120]); fail+=1
        time.sleep(0.5)
